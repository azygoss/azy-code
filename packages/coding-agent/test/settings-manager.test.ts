...
describe("SettingsManager", () => {
	...
	beforeEach(() => {
		...
		mkdirSync(join(projectDir, ".azycode"), { recursive: true });
	});
	...
	describe("error tracking", () => {
		it("should collect and clear load errors via drainErrors", () => {
			...
			const projectSettingsPath = join(projectDir, ".azycode", "settings.json");
	...
	describe("project settings directory creation", () => {
		it("should not create .azycode folder when only reading project settings", () => {
			// Create agent dir with global settings, but NO .azycode folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));

			// Delete the .azycode folder that beforeEach created
			rmSync(join(projectDir, ".azycode"), { recursive: true });

			// Create SettingsManager (reads both global and project settings)
			const manager = SettingsManager.create(projectDir, agentDir);

			// .azycode folder should NOT have been created just from reading
			expect(existsSync(join(projectDir, ".azycode"))).toBe(false);

			// Settings should still be loaded from global
			expect(manager.getTheme()).toBe("dark");
		});

		it("should create .azycode folder when writing project settings", async () => {
			// Create agent dir with global settings, but NO .azycode folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));
			...
			// .azycode folder should NOT exist yet
			expect(existsSync(join(projectDir, ".azycode"))).toBe(false);
			...
			// Now .azycode folder should exist
			expect(existsSync(join(projectDir, ".azycode"))).toBe(true);

			// And settings file should be created
			expect(existsSync(join(projectDir, ".azycode", "settings.json"))).toBe(true);
		});
	});
	...
	describe("httpIdleTimeoutMs", () => {
		...
		it("should use merged global and project settings", () => {
			...
			writeFileSync(join(projectDir, ".azycode", "settings.json"), JSON.stringify({ httpIdleTimeoutMs: 0 }));
	...
	describe("getSessionDir", () => {
		...
		it("should return project sessionDir, overriding global", () => {
			...
			writeFileSync(join(projectDir, ".azycode", "settings.json"), JSON.stringify({ sessionDir: "./sessions" }));
			const manager = SettingsManager.create(projectDir, agentDir);
