...
Content`,
		...
		it("should auto-discover root markdown skills from .azycode skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .azycode", async () => {
			const extDir = join(tempDir, ".azycode", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".azycode"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".azycode", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".azycode", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".azycode", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".azycode", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".azycode", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .azycode/agent skills", async () => {
		...
		it("should use the project .azycode dir as baseDir for project .azycode skills", async () => {
			const projectBaseDir = join(tempDir, ".azycode");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".azycode", "agent");
		...
		it("should dedupe user skill entries when ~/.azycode/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .azycode auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".azycode\n");

			const skillDir = join(tempDir, ".azycode", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.azycode\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".azycode", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .azycode settings base", () => {
			...
			const rel = relative(join(tempDir, ".azycode"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			const installedPath = join(tempDir, ".azycode", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".azycode", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".azycode", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".azycode", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".azycode", "npm", "node_modules", "example");
