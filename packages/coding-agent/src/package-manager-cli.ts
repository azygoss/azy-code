...
Update ${APP_TITLE} and installed packages.

Options:
  --self                  Update ${APP_TITLE} only
  --extensions            Update installed packages only
  --extension <source>    Update one package only
  --force                 Reinstall ${APP_TITLE} even if the current version is latest

Short forms:
  ${APP_NAME} update                Update ${APP_TITLE} and all extensions
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update ${APP_NAME}    Update ${APP_TITLE} only (self works as alias to ${APP_NAME})
`);
			return;
...
function printSelfUpdateUnavailable(npmCommand?: string[], updatePackageName = PACKAGE_NAME): void {
	...
	if (entrypoint) {
		console.error("");
		console.error(`Location of ${APP_TITLE} executable: ${entrypoint}`);
	}
}
